...
1. **[Download Refhaven-0.1.4-arm64.dmg](https://github.com/Hongxiang2023/folio/releases/download/v0.1.4-preview.1/Refhaven-0.1.4-arm64.dmg)** from this project's GitHub release.
...
Refhaven is the new name for Folio, starting with version 0.1.4. Your existing library, cloud-folder selection, annotations, citation styles, and saved credentials use the same locations and internal identity.
...
5. If you reload the unpacked browser connector from Refhaven's new application path, pair it again using **Library & connector → Copy connector pairing key**. Remove the old connector entry after the new one works. An already-loaded Folio connector uses the same protocol and remains compatible.
...
The GitHub repository and website retain their existing `/folio` addresses for link continuity while the visible product name is Refhaven.
...
Download the [Refhaven source ZIP](https://github.com/Hongxiang2023/folio/archive/refs/heads/main.zip) and extract it. Find the folder containing `package.json` — normally `folio-main`. Keep that folder somewhere you can find again, such as Documents.
...
2. Double-click the downloaded Refhaven ZIP to extract it. Move `folio-main` into Documents if you want to keep it there.
