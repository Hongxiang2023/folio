...
export function bib(p:Paper){const esc=(s:string)=>s.replace(/[\\{}%&#_$^~]/g,c=>({'\\':'\\textbackslash{}','^':'\\textasciicircum{}','~':'\\textasciitilde{}'}[c]||'\\'+c));return `@${p.cslType==='paper-conference'?'inproceedings':p.cslType==='article'?'misc':'article'}{ref${Array.from(p.id).map(c=>c.codePointAt(0)!.toString(16)).join('x')},\n${Object.entries({title:p.title,author:p.authors.split(';').map(a=>a.trim()).filter(Boolean).join(' and '),year:p.year,[p.cslType==='paper-conference'?'booktitle':'journal']:p.journal,doi:citationDoi(p),volume:p.volume||'',number:p.issue||'',pages:p.pages||'',url:referenceUrl(p,{allowPubMed:true})}).filter(([,v])=>v).map(([k,v])=>`  ${k} = {${esc(v)}}`).join(',\n')}\n}`;}
...
export function importReferences(text:string,format:string):Paper[]{
 if(format==='json'){const data:unknown=JSON.parse(text);if(!Array.isArray(data)||!data.every(valid))throw Error('Choose a Refhaven reference backup.');return data.map(p=>{const copy={...p,collection:p.collection.trim()||'Unfiled'};delete copy.pdfId;delete copy.pdfName;return copy;});}
 ...
 throw Error('Choose a .ris or Refhaven .json reference file.');
}
